...
jQuery.validator.addMethod("rate", function(value, element) {
	return this.optional(element) || /^((^(-|\+)?[1-9]{1}\d*(\.\d{1})?)|(0)|(0\.\d{1}))$/.test(value);
}, "比例保留一位小数");
...
jQuery.validator.addMethod("moneyNotZero", function(value, element) {
	return this.optional(element) || /^(([1-9]{1}\d*(\.\d{1,2})?)|(0\.([0][1-9]?|[1-9][0-9]?)))$/.test(value);
}, "金额必须大于0.00，保留两位小数");

jQuery.validator.addMethod("moneyNotOne", function(value, element) {
	return this.optional(element) || /^(([1-9]{1}\d*(\.\d{1,2})?)|(0\.([1-9][0-9]?)))$/.test(value);
}, "金额必须大于0.10，保留两位小数");
...
jQuery.validator.addMethod("moreAndEqualTo", function(value, element, param) {
 	var falg = Number(value)  >= Number($(param).val());
 	if(falg){
 		var name=param.substr(1);
 		$(param).parent().find("label[for='"+name+"']").css({display : "none"});
 	}
	return falg;
}, "必须大于等于{0}");
/** 两字段对比,是否小于*/
...
/** 两字段对比,是否小于*/
jQuery.validator.addMethod("lessTonew", function(value, element, param) {
	var maxnum = $(param).val()?$(param).val():$(param).text();
	return value < parseFloat(maxnum);
}, "必须小于{0}");
...
jQuery.validator.addMethod("amoutLessAndEqualTo", function(value, element, param) {
	var val = $(param).val();
	if(value < 0){
		return val>=Math.abs(value);
	}
	return true;
}, "必须小于等于{0}");

/** 判断用户密码输入是否正确*/
jQuery.validator.addMethod("truePassword", function(value, element, param) {
...
//英文字母
 ...
 // 地区验证
 jQuery.validator.addMethod("vareaCode", function(value, element) {
   return this.optional(element) || (0!=value);
 }, "请选择地址");
...
//开始日期必须大于当前时间后的多长时间
jQuery.validator.addMethod("moreThenTime",function(value,element,param){
	var date2 = new Date().getTime()+param;
	value = value.replace(/-/g,"/");
	var date1 = new Date(value).getTime();
        return date2<date1;
}, "开始日期必须大于当前时间!");
//电话号(包含手机跟座机)码验证  
jQuery.validator.addMethod("telAndphone", function(value, element) {  
var tel = /^(([0\+]\d{2,3}-)?(0\d{2,3})-)(\d{7,8})(-(\d{3,}))?$|^(1[35784])\d{9}$|^((4\d{2}-)(\d{3})-)(\d{4})$/;
return this.optional(element) || tel.test(value);  
}, "请输入正确的手机号码或电话号码\n\n例如:18123456789或0571-1234567或400-000-3777");  

//开始日期必须大于当前时间后的多长时间
jQuery.validator.addMethod("integerzero",function(value,element){
	var reg = /^([0-9])*$/;
