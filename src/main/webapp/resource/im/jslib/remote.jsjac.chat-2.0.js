...
var remote = {
    ...
    checkContent :false,
    isLoad:false,//是否加载过，如果加载过，就不在加载
    console: {
        errorEL: function () {
    ...
    userAddress: function (user) {
        if (user) {
            if (!~user.indexOf("@")) {
                user += "@";
                user += null!=remote.jsjac.clusterdomain?remote.jsjac.clusterdomain:remote.jsjac.domain;// + "/" + remote.jsjac.resource;
    ...
    jsjac: {
        ...
        clusterdomain: window["clusterdomain"],//集群域
        username: "",
        pass: "",
...
remote.jsjac.chat = {
    ...
    login: function (loginForm) {
        ...
        try {
            ...
            if (loginForm) {
            	...
            	//设置集群域
                connectionConfig.clusterdomain = remote.jsjac.clusterdomain;
                if("1"==loginForm.register.value){
                	connectionConfig.register=true;
                }else{
    ...
    loginAnonymously: function () {
        ...
        try {
        	...
        	connectionConfig.register=false;
        	//设置为匿名用户类型
        	connectionConfig.authtype = "saslanon";
        	//设置集群域
        	connectionConfig.clusterdomain = remote.jsjac.clusterdomain;
            // 连接服务器
            connection.connect(connectionConfig);
    ...
    writeStates : function (content){
    	if (remote.jsjac.chat.writeSystemMessage && !!content) {
            if(!remote.isLoad){
            	remote.jsjac.chat.writeSystemMessage(content);
            	remote.isLoad=true;
            }
        }
    },
...
//        remote.console.info(html);
    ...
    handlePresence: function (aJSJaCPacket) {
        ...
        if (!aJSJaCPacket.getType() && !aJSJaCPacket.getShow()) {
        	remote.checkContent = true;
            html += "<d>" + userName + " 上线了.</b>";
            states = 1;
        } else {
        	states = 2;
        ...
        remote.jsjac.chat.writeStates(states);
    },
...
//        remote.console.info("<div>当前用户状态: " + status + "</div>");
        ...
        if (status == "disconnecting") {
        	...
        	remote.jsjac.chat.writeStates(2);
        }
    },
    // 建立链接触发事件方法
