						...
						<div class="box">
							<div class="title">
								<ul class="nav nav-tabs">
									...
									<li class="active"><a href="${ctx}/u/stuff/article/listCategory${ext}">文章专题管理</a></li>
									<li class=""><a href="${ctx}/u/stuff/article/listColumn${ext}">文章栏目管理</a></li>
								</ul>
							</div>
							<div class="content">
								<div class="alert alert-warning" role="alert">
									<p>将文章组合，形成专题达到更好的展示效果</p>
								</div>
								<ul class="pager">
									<li class="previous">
										<a href="${ctx}/u/stuff/article/addCategory${ext}" class="btn btn-default">添加专题</a>
									</li>
								</ul>
								<form action="${ctx}/u/stuff/article/listCategory${ext}"  method="post" id="formPage">
								...
								<table class="table table-bordered" id="template">
									...
									<tbody>
										...
										<tr>
											...
											<td><p>${data.articleCount }</td>
											<td>文章模板${data.articleTemplateType}</td>
											<td>
												<a href="javascript:void(0)" class="btn-editor" onclick="editCategory(this,'${data.id}')">修改</a> | 
...
<%-- 												<a href="javascript:void(0)" class="btn-del" onclick="preview('${data.id }')"> | 预览</a> --%>
	...
	<form action="" id="editCategory" method="get">
...
//预览
function preview(id) {
	var url= '${ctx}/any/preview/articleCategory${ext}?id='+id;
...
function del(el,id) {
	art.dialog.confirm('确认删除？', function() {
		$.post("${ctx}/u/stuff/article/delCategory${ext}",{"id":id},function(data){
			if(data=="SUCCESS"){
...
function editCategory(thiz,id){
		$("#editCategory").attr("action","${ctx}/u/stuff/article/editCategory${ext}");
		$("#editId").val(id);
		$("#editCategory").submit();
}
