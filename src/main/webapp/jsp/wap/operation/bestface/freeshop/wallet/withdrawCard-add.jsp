...
<body>
	...
	<script type="text/javascript">
		...
		function cardbin(cardNo) {
			...
			$.post("${ctx}/free/wallet/withdraw/cardbin${ext}", 
		...
		function formSubmit() {
			if (validForm.form()) {
				$.post("${ctx}/free/wallet/withdraw/add${ext}", 
						$('#jform').serialize(), function(jsonData){
					console.log(jsonData);
					var data = JSON.parse(jsonData);
					if (data.success) {
						var id = data.info;
						window.location.href="${ctx}/free/wallet/withdraw/withdraw${ext}?from=add&id="+id;
					} else {
						art.dialog.alert(data.info);
					}
				}); 
