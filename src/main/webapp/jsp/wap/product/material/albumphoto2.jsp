...
<link rel="stylesheet" href="${resourcePath}template/page/photo2/css/main.css">
</head>
<style>
#slider{
	...
	<div id="content">
		<c:forEach items="${pagination.datas }" var="album">
			<div class="item">
				...
				<div class="title">
					<h3>${album.name }</h3>
					<span><i class="iconfont icon-pullright"></i></span>
				</div>
			</div>
		</c:forEach>
	</div>
...
function getAppendHtml(dataArray){
	...
	for(x in dataArray){
   		var str = 
   			'<div class="item">'+
				'<div class="img"><a href="${ctx }/wap/{shopNo}/any/album/photolist${ext}?albumId='+dataArray[x].id+'"><img src="'+ROOT_PICURL+dataArray[x].path+'" alt=""></a></div>'+
				'<div class="title">'+
					'<h3>'+dataArray[x].name+'</h3>'+
					'<span><i class="iconfont icon-pullright"></i></span>'+
				'</div>'+
			'</div>';
		htm_str+=str;
