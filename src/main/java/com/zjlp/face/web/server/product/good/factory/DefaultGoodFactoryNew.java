...
/**
 * 四阶段商品改动新建工厂:单属性商品,双属性商品,三属性商品
 * @ClassName: DefaultGoodFactoryNew 
 * @Description: (这里用一句话描述这个类的作用) 
 * @author talo
 * @date 2015年7月7日 下午3:33:30
 */
@Component("defaultGoodFactoryNew")
public class DefaultGoodFactoryNew implements GoodFactory {
	...
	@Autowired
	private ElementFactory<List<GoodProperty>> goodPropertyElementFactoryNew;
	...
	public void addGood(GoodParam goodParam) throws GoodException {
		try {
			...
			/** step2 创建商品规格属性*/
			goodPropertyElementFactoryNew.create(goodParam);
	...
	public void editGood(GoodParam goodParam) throws GoodException {
		try {
			...
			/** step2 修改商品属性*/
			goodPropertyElementFactoryNew.edit(goodParam);
