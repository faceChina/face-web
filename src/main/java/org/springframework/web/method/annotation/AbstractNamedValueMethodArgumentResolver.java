...
public abstract class AbstractNamedValueMethodArgumentResolver implements HandlerMethodArgumentResolver {
	...
	/**
	 * @param beanFactory
	 *            a bean factory to use for resolving ${...} placeholder and
	 *            #{...} SpEL expressions in default values, or {@code null} if
	 *            default values are not expected to contain expressions
	 */
...
//		boolean emptyArgValue = "".equals(arg);
	...
	private NamedValueInfo updateNamedValueInfo(MethodParameter parameter, NamedValueInfo info) {
		...
		if (info.name.length() == 0) {
			...
			Assert.notNull(name, "Name for argument type [" + parameter.getParameterType().getName() + "] not available, and parameter name information not found in class file either.");
		}
	 ...
	 * @param mavContainer
	 *            the {@link ModelAndViewContainer}, which may be {@code null}
	 * @param webRequest
	 *            the current request
	 */
